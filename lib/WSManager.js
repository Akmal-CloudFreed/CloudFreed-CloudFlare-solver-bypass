        ...
        try {
          ...
          if (response.method === 'Network.requestIntercepted') {
            ...
            if (response.result && response.result.result && response.result.result.value === 'true') {
              ws.send(JSON.stringify({ sessionId: response.sessionId, id: 10, method: 'Page.reload' }))
              await delay(1000)
            ...
            resolve({ success: true, json: formattedCookies, header: cookieHeader, cf_clearence });
          }
        } catch (error) {
          if (ws.readyState === 1) {
            ws.close();
